...
/* Read or Write all data from/to a file descriptor. Retry on EINTR or EAGAIN.
   ...
   errno. */
extern ssize_t read_all(int fd, void *buf, size_t buflen);
extern ssize_t write_all(int fd, const void *buf, size_t buflen);
...
extern int read_file(const char *path, void **buf, size_t *buflen);
