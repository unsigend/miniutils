...
UTEST_CASE(copy_file)
{
  ...
  WRITE_FILE(src, "ok");
  EXPECT_EQ_INT(copy_file(src, dst), 0);
  ...
  TMP_PATH(dst, "copy");
  EXPECT_EQ_INT(copy_file("Makefile", dst), 0);
  EXPECT_TRUE(file_exists(dst));
  EXPECT_EQ_INT(unlink(dst), 0);
  ...
  TMP_PATH(dst, "nope");
  EXPECT_FAIL(copy_file(src, dst), ENOENT);
  ...
  EXPECT_FAIL(copy_file("Makefile", dst), ENOENT);

  TMP_PATH(dst, "dir");
  EXPECT_EQ_INT(mkdir(dst, 0755), 0);
  EXPECT_FAIL(copy_file("Makefile", dst), EISDIR);
  EXPECT_EQ_INT(rmdir(dst), 0);

  TMP_PATH(dst, "out");
  errno = 0;
  EXPECT_EQ_INT(copy_file("build", dst), -1);
...
UTEST_CASE(copy_dir)
{
  ...
  EXPECT_EQ_INT(mkdirp(dst, 0755), 0);
  TMP_PATH(src, "missing");
  EXPECT_FAIL(copy_dir(src, dst), ENOENT);
  EXPECT_EQ_INT(rmdir(dst), 0);
  ...
  TMP_PATH(path, "file");
  TOUCH_FILE(path);
  EXPECT_FAIL(copy_dir(path, dst), ENOTDIR);
  ...
  TOUCH_FILE(dst);
  EXPECT_EQ_INT(mkdirp(src, 0755), 0);
  EXPECT_FAIL(copy_dir(src, dst), ENOTDIR);
  EXPECT_EQ_INT(unlink(dst), 0);
  EXPECT_EQ_INT(rmdir(src), 0);
