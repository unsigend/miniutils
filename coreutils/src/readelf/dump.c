...
void elf_dump_section(_elf_meta *elf_file, const char *sh_name_idx) {
  ...
  if (endptr && *endptr == '\0') {
    if (sh_idx == 0)
      ...
      fprintf(stderr,
              "readelf: Warning: Section '%ld' was not dumped because it "
              "does not exist\n",
              sh_idx);
    else {
      if (elf_file->elf_class == ELFCLASS64) {
        if (elf_file->shstrtab &&
            elf_file->shdr.shdr64[sh_idx].sh_name < shstrsize) {
        ...
        if (elf_file->shstrtab &&
            elf_file->shdr.shdr32[sh_idx].sh_name < shstrsize) {
  ...
  // section name
    ...
    for (uint16_t i = 0; i < shnum; i++) {
      ...
      if (elf_file->elf_class == ELFCLASS64) {
        if (elf_file->shstrtab &&
            elf_file->shdr.shdr64[i].sh_name < shstrsize) {
          current_sh_name =
              &elf_file->shstrtab[elf_file->shdr.shdr64[i].sh_name];
        }
        ...
        if (elf_file->shstrtab &&
            elf_file->shdr.shdr32[i].sh_name < shstrsize) {
          current_sh_name =
              &elf_file->shstrtab[elf_file->shdr.shdr32[i].sh_name];
        }
        if (current_sh_name && !strcmp(current_sh_name, sh_name_idx)) {
          sh_name = current_sh_name;
  ...
  if (!sh_name || sh_offset == 0) {
    fprintf(stderr,
            "readelf: Warning: Section '%s' was not dumped because it does "
            "not exist\n",
