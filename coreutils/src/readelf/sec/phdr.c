...
__DEP_EHDR(elf_file);
    ...
    if (phnum == 0 && phsize == 0) {
        ...
        else elf_file->phdr.phdr32 = NULL;
...
__DEP_PHDR(elf_file);
    ...
    else e_type = elf_file->ehdr.ehdr32.e_type;

    printf("Elf file type is %s\n", get_elf_type(e_type));
    printf("Entry point 0x%lx\n", elf_file->elf_class == ELFCLASS64 ? 
        elf_file->ehdr.ehdr64.e_entry : (uint64_t)elf_file->ehdr.ehdr32.e_entry);
    ...
    for (uint16_t i = 0; i < phnum; i++) {
        ...
        printf("  %-14s 0x%016lx 0x%016lx 0x%016lx\n",
            type_name, p_offset, p_vaddr, p_paddr);
